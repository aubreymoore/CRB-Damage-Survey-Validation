...
The confusion matrix is generated by [this Jupyter notebook](todo).

## RESULTS AND DISCUSSION

![](/home/aubrey/Desktop/CRB-Damage-Survey-Validation/confusion_matrix.png) 

With an overall accuracy of 51%, results from automated assignment of the CRB damage index to tree images are disappointing.  The biggest problem is an extremely high false positive rate for CRB damage detection. The human labeled 
...
Trees with zero damage should not contain any v-shaped cuts.
We can count the number of v-shaped cuts with the bounding box for each tree object.

* for each frame:
    * for each tree within frame:
        * count the number of v-shaped cuts within the bounding_box for the tree
